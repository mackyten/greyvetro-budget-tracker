...
/// Opens the add/edit form as a draggable-feeling modal sheet, styled like
/// `manage_accounts_screen.dart`'s add-account sheet.
Future<void> showVaultItemEditorSheet(
  ...
  return showModalBottomSheet<void>(
    ...
    builder: (_) => VaultItemEditorSheet(existing: existing, accounts: accounts),
  );
}

class VaultItemEditorSheet extends StatefulWidget {
  const VaultItemEditorSheet({super.key, this.existing, this.accounts = const []});
...
class _VaultItemEditorSheetState extends State<VaultItemEditorSheet> {
  ...
  void initState() {
    ...
    _cardholderNameController = TextEditingController(text: e?.cardholderName ?? '');
  ...
  Future<void> _save() async {
    ...
    final item = VaultItem(
      ...
      kind: _kind,
      title: title,
      accountNumber: _kind == VaultItemKind.bankCard ? orNull(_accountNumberController.text) : null,
      cardNumber: _kind == VaultItemKind.bankCard ? orNull(_cardNumberController.text) : null,
      cardholderName:
          _kind == VaultItemKind.bankCard ? orNull(_cardholderNameController.text) : null,
                  ...
                  labeled(
                    ...
                    TextField(
                      ...
                      decoration: decoration(
                        _kind == VaultItemKind.bankCard ? 'e.g. Metrobank Visa' : 'e.g. Wifi password',
                      ),
                    ),
                  ),
